...
# canvas item types.
#
# RCS: @(#) $Id: items.tcl,v 1.1 2004/08/20 00:04:51 bruce_johnson Exp $
# removed stipple
...
canvas $c -scrollregion {0c 0c 30c 24c} -width 15c -height 10c -relief sunken -borderwidth 2 -xscrollcommand "$w.frame.hscroll set" -yscrollcommand "$w.frame.vscroll set"
...
$c create rect 0c 0c 30c 24c -width 2 -bg orange
...
$c create line 5c 6c 9c 6c 9c 1c 8c 1c 8c 4.8c 8.8c 4.8c 8.8c 1.2c \
	8.2c 1.2c 8.2c 4.6c 8.6c 4.6c 8.6c 1.4c 8.4c 1.4c 8.4c 4.4c -width 3 -fill $red -tags item
...
$c create rectangle 6c 10c 9c 15c -outline {red} \
	-fill $blue -tags item
...
# BAJ modified for swank to get current item and then use explicit item number
# rather than "current" in the restoreCmd
proc itemEnter {c} {
    global restoreCmd
    ...
    if {$type == "bitmap"} {
	set bg [lindex [$c itemconf $item -background] 4]
	set restoreCmd [list $c itemconfig $item -background $bg]
	$c itemconfig $item -background SteelBlue2
	return
    }
    set fill [lindex [$c itemconfig $item -fill] 4]
    if {(($type == "rectangle") || ($type == "oval") || ($type == "arc"))
	    && ($fill == "")} {
	set outline [lindex [$c itemconfig $item -outline] 4]
	set restoreCmd "$c itemconfig $item -outline $outline"
	$c itemconfig $item -outline SteelBlue2
    } else {
	set restoreCmd "$c itemconfig $item -fill $fill"
	$c itemconfig $item -fill SteelBlue2
    }
}
