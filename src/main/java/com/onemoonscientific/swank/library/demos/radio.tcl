...
#
# RCS: @(#) $Id: radio.tcl,v 1.1 2004/08/20 00:04:52 bruce_johnson Exp $
...
button $w.buttons.vars -text "See Variables"  	-command "showVars $w.dialog size color"
...
foreach i {10 12 18 24} {
    radiobutton $w.left.b$i -text "Point Size $i" -variable size  -relief flat -value $i
    pack $w.left.b$i  -side top -pady 2 -anchor w
}

foreach color {Red Green Blue Yellow Orange Purple} {
    set lower [string tolower $color]
    radiobutton $w.right.$lower -text $color -variable color 	    -relief flat -value $lower
