...
#
# RCS: @(#) $Id: floor.tcl,v 1.2 2005/11/30 17:54:49 bruce_johnson Exp $
...
proc roomChanged {w args} {
    ...
    set new [eval "$w create polygon [$w coords $item] -fill $colors(active) -tags highlight"]
    $w raise $new marker
}
...
proc bg2 {w fill outline} {
	...
	367 802 367 802 129 725 129 725 133 559 133 559 129 -tags {floor2 bg} -fill $fill
...
proc bg3 {w fill outline} {
	...
	133 21 133 21 331 0 331 0 391 60 391 60 370 159 370 159 300 -tags {floor3 bg} -fill $fill
    $w create polygon 258 370 258 329 350 329 350 311 399 311 399 129 \
	315 129 315 133 176 133 176 129 159 129 159 370 258 370 -tags {floor3 bg} -fill $fill
    ...
    $w create line 107 300 159 300 159 248 107 248 107 300 -fill $outline -tags {floor3 bg}
}
...
entry $c.entry -width 10 -relief sunken -bd 2 -textvariable currentRoom -bg red
