...
proc drawItems {c type startCoords dX attrs} {
puts "$type $attrs"
    set appAttr [list]
    set x 0
...
toplevel $w
wm title $w "Canvas GeomObjects Demonstration"
wm iconname $w "Items"
positionWindow $w
set c $w.frame.c

label $w.msg -font $font -wraplength 5i -justify left -text "This window contains a canvas widget showing some of the attributes that canvas items can have."
pack $w.msg -side top
...
drawItems $c line $coords $dX $attrs
...
set coords [incrCoords $lineCoords "0 $y"] 
drawItems $c symbols $coords $dX $attrs
...
drawItems $c rectangle $coords $dX $attrs
...
drawItems $c oval $coords $dX $attrs
