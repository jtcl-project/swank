...
##
## Modified by Bruce Johnson to work with Swank
...
proc ::tkcon::Init {args} {
    ...
    foreach {key default} {
        ...
        RCS		{RCS: @(#) $Id: swkcon.tcl,v 1.6 2005/11/15 20:06:59 brucejohnson Exp $}
    ...
    if {![info exists ::tcl_pkgPath]} {
        ...
        set pkgIndexFile  [file join $dir pkgIndex.tcl]
        if {[file exists $pkgIndexFile]} {
            catch {source $pkgIndexFile}
        }
    }
    ...
    if {[string compare {} $OPT(exec)]} {
        ...
        # FIXME  uplevel crashes on java if any slaveargs
...
proc ::tkcon::InitUI {title} {
    ...
    ## Create console tab
    set jtab [jtabbedpane $w.jtab]
    set con [InitTab $jtab]
    set PRIV(curtab) $con
    ...
    #    set PRIV(scrolly) $sy
    bind $jtab <<StateChanged>> "::tkcon::GotoTab %W %s"
    
    ## Menus
    ## catch against use in plugin
    if {$OPT(showmenu)} {
        if {[catch {jmenubar $w.mbar} PRIV(menubar)]} {
            puts "errormenu $PRIV(menubar)"
            set PRIV(menubar) [frame $w.mbar -relief raised -bd 1]
        }
    } else {
        menu .mbar.console
        set PRIV(menubar) .mbar.console
    }
    ...
    grid $jtab  -row 1 -column 1 -sticky news
    ...
    set OPT(showstatusbar) 1
    if {!$OPT(showstatusbar)} {
        grid remove $sbar
    }
    if {!$PRIV(WWW)} {
        wm title $root "swkcon $PRIV(version) $title"
...
proc ::tkcon::InitTab {w} {
    ...
    variable COLOR
    # text console
    set fr [frame $w.tab[incr PRIV(uid)] -padx 5 -pady 5]
    $w add $fr $PRIV(uid)
    ...
    if {!$PRIV(WWW)} {
        ...
        $con configure  -width $OPT(cols) -height $OPT(rows)
        ...
        #	}
        #	bind $con <Configure> {
        #	    regexp {(0-9)+x(0-9)+} [wm geometry [winfo toplevel %W]] all ::tkcon::OPT(cols) ::tkcon::OPT(rows)
        #	}
    }
    ...
    $con tag configure insert -foreground blue
    
    if {$::tcl_version >= 8.4} {
        #$rb configure -offrelief flat -overrelief raised
    }
    jscrollpane $spane
    pack  $spane -fill both -expand y
    $spane add $con
    lappend PRIV(tabs) $con
    return $con
}

proc ::tkcon::GotoTabOld {con} {
    variable PRIV
    variable ATTACH
...
proc ::tkcon::GotoTab {con index} {
    variable PRIV
    variable ATTACH
    set con [lindex $PRIV(tabs) $index]
    if {$con == $PRIV(console)} {
        return
    }
    
    # adjust console
    if {[info exists PRIV(console)] && [winfo exists $PRIV(console)]} {
        set ATTACH($PRIV(console)) [Attach]
    }
    set PRIV(console) $con
...
proc ::tkcon::NewTab {{con {}}} {
    ...
    set con [InitTab $PRIV(base).jtab]
...
proc ::tkcon::Prompt {{pre {}} {post {}} {prompt {}}} {
    ...
    if {[string compare {} $prompt]} {
        ...
        ::tkcon::sayString $prompt prompt
    } else {
        set newPrompt [EvalSlave subst $OPT(prompt1)]
        $w insert end $newPrompt
        ::tkcon::sayString $newPrompt prompt
    }
    $w mark set output $i
...
proc ::tkcon::InitMenus {w title} {
    ...
    if {[catch {menu $w.pop -tearoff 0} err]} {
    ...
    foreach m [list [menu $w.file -disabledforeground $COLOR(disabled)] \
        ...
        $m add separator
        $m add command -label "Quit" -underline 0 -accelerator Ctrl-q -command exit
    ...
    ## Console Menu
    ##
    foreach m [list [menu $w.console -disabledforeground $COLOR(disabled)] \
            [menu $w.pop.console -disabledforeground $COLOR(disabled)]] {
        $m add command -label "$title Console"	-state disabled
        $m add command -label "New Console"	-underline 0 -accelerator Ctrl-N \
                -command ::tkcon::New
        $m add command -label "New Tab"		-underline 4 -accelerator Ctrl-T \
                -command ::tkcon::NewTab
        $m add command -label "Close Console"	-underline 0 -accelerator Ctrl-w \
                -command ::tkcon::Destroy
        $m add command -label "Clear Console"	-underline 1 -accelerator Ctrl-l \
    ...
    foreach m [list [menu $w.edit] [menu $w.pop.edit]] {
        $m add command -label "Cut"   -underline 2 -accelerator Ctrl-x \
                -command [list ::tkcon::Cut $text]
        $m add command -label "Copy"  -underline 0 -accelerator Ctrl-c \
                -command [list ::tkcon::Copy $text]
        $m add command -label "Paste" -underline 0 -accelerator Ctrl-v \
                -command [list ::tkcon::Paste $text]
        $m add separator
        $m add command -label "Find"  -underline 0 -accelerator Ctrl-F \
    ...
    foreach m [list [menu $w.help] [menu $w.pop.help]] {
        $m add command -label "About " -underline 0 -accelerator Ctrl-A \
                -command ::tkcon::About
...
proc ::tkcon::AttachMenu m {
    ...
    $m add radio -label {None (use local slave) } -accelerator Ctrl-1 \
    ...
    foreach i [lsort [array names interps]] {
        ...
        if {[regexp {^Slave[0-9]+} $i]} {
            ...
            if {[string match $PRIV(name) $i]} {
                append opts " -accelerator Ctrl-2"
            }
            eval $m add radio $opts
        } else {
            set name [concat Main $i]
            if {[string match Main $name]} {
                $m add radio -label "$name ($interps($i))" -accelerator Ctrl-3 \
...
proc ::tkcon::MainInit {} {
    ...
    proc ::tkcon::InterpEval {{slave {}} args} {
        ...
        if {[llength $args]} {
            ...
            return [interp eval $slave uplevel \#0 [lindex $args 0]]
        } else {
        ...
        if {$int > 0} {
            ...
            if {$PRIV(event) < $nextid} {
                ...
                if {[incr PRIV(event)] == $nextid} {
                    $w insert limit $PRIV(cmdbuf)
                    ::tkcon::sayString $PRIV(cmdbuf)
                } else {
                    $w insert limit [EvalSlave history event $PRIV(event)]
                    ::tkcon::sayString [EvalSlave history event $PRIV(event)]
                }
            }
        } else {
...
proc ::tkcon::sayStringNow {} {
    variable PRIV
    set speak $PRIV(speechOn)
    set dest $PRIV(sayDest)
...
proc ::tkcon::sayStringNowFreeTTS {} {
    variable PRIV
    set dest $PRIV(sayDest)
    set s $PRIV(sayString)
    if {![info exists PRIV(voice)]} {
        set vm [java::call com.sun.speech.freetts.VoiceManager getInstance]
        set PRIV(voice) [$vm getVoice kevin16]
        $PRIV(voice) allocate
    }
    set voice $PRIV(voice)
    if {$dest ne ""} {
        $voice speak $dest
    }
    $voice speak $s
...
proc edit {args} {
    ...
    if {[string compare $opts(-type) {}]} {
        ...
        $m add separator
        $m add command -label "Dismiss" -underline 0 -accelerator "Ctrl-w" \
                -command [list destroy $w]
...
proc ::tkcon::Bindings {} {
    ...
    proc ::tkcon::ClipboardKeysyms {copy cut paste} {
        bind TkConsole <$copy>	{::tkcon::Copy %W;break}
        bind TkConsole <$cut>	{::tkcon::Cut %W;break}
        bind TkConsole <$paste>	{::tkcon::Paste %W;break}
    }
    ...
    proc ::tkcon::Copy w {
        ...
        clipboard clear
            ...
            set txt [$w get sel.first sel.last]
            clipboard append -displayof $w $txt
        }
    }
    proc ::tkcon::Paste w {
    ...
    proc ::tkcon::TkConSetCursor {w pos} {
    ...
    if {[string match Mac* $tcl_platform(os)]} {
        ::tkcon::ClipboardKeysyms Meta-c Meta-x Meta-v
    } else {
        ::tkcon::ClipboardKeysyms Control-c Control-x Control-v
    }
    ...
    bind TkConsole <Left> {
        ::tkcon::TkConSetCursor %W insert-1c
    }
    bind TkConsole <Right> {
        ::tkcon::TkConSetCursor %W insert+1c
    }
    ...
    bind TkConsole <<TkCon_Previous>> {
        if {[%W compare {insert linestart} != {limit linestart}]} {
            #	    tkTextSetCursor %W [::tkcon::TextUpDownLine %W -1]
        } else {
            ::tkcon::Event -1
        }
    }
    ...
    bind TkConsole <$PRIV(meta)-Up> {
        catch { ::tkcon::sayOutputLine %W 1}
    }
    bind TkConsole <$PRIV(meta)-Down> {
        catch { ::tkcon::sayOutputLine %W -1}
    }
    ...
    bind TkConsole <ButtonRelease-2> {
        if {[info exists tkPriv(mouseMoved)] && (!$tkPriv(mouseMoved) || $tk_strictMotif) &&
